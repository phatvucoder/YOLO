...
def group_anchors(anchors, num_groups=3):
    """
    Group anchors into `num_groups` based on area size.
    
    Args:
        anchors (array): Array of anchor box dimensions (width, height).
        num_groups (int): Number of groups (e.g., 3 for YOLOv3).
    
    Returns:
        List of grouped anchors, sorted by area.
    """
    areas = anchors[:, 0] * anchors[:, 1]
    sorted_indices = np.argsort(areas)
    sorted_anchors = anchors[sorted_indices]
    grouped = np.array_split(sorted_anchors, num_groups)
    return [group.tolist() for group in grouped]
...
if __name__ == "__main__":
    ...
    # Group anchor boxes into pairs of similar sizes
    anchor_groups = group_anchors(anchors)
    print("Anchor Pairs:")
    for i, group in enumerate(anchor_groups):
        print(f"Scale {i + 1} Anchors: {group}")
